...
class DFPlayerMini ():
    ...
    #------below is the version for playing tracks with no folders
    ...
    def play(self, folder, track):
        ...
        self.send_command(0x16)  # optional: send stop command (22 decimal)
        time.sleep(0.05)
    
        # Play the track in folder
        return_value = self.send_command(0x0F, (folder << 8) + track)  
        # DFPlayer expects para1 = high byte, para2 = low byte; combining folder and track
        if return_value is None or len(return_value) != 10:
            if self.debug:
                print(f"Play command failed for folder {folder}, track {track}")
            return False
        return True
